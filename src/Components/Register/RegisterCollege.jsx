...
const RegisterCollege = () => {
    ...
    const [exColleges, setexColleges] = useState('')
    ...
    const handleSubmit = (event) => {
        ...
        if (name == 'email' && emails.includes(value)) {
            toast.error("The emails is alredy exist")
            console.log("The emails is alredy exist")
    ...
    const handleCollegeChange = (event) => {
        ...
        if (exColleges.filter((item) => { return item.trim() === event.target.value.trim() }).length != 0) {
            toast.error("The college is alredy Registerd")
            event.preventDefault();
            return;
        }
    ...
    const checkcolleges = async () => {
        try {
            const response = await userService.ExsitingEmails()
            setEmails(response?.map((item, index) => (
                item?.email
            )))
            setexColleges(response?.map((item) => (
                item?.collegename
            )))
        } catch (error) {
            console.log(error)
    ...
    console.log("The selected state is this", state)
        ...
        <>
            {loadingPage ? (<Spinner />) :
                (<Layout title="| Register |" content="content">
                    <section className="bg-gray-100 h-[800px]">
                                        ...
                                        <div>
                                            ...
                                            >
                                                {states.map((state,index) => (
                                                    <option key={index+1} value={state && state}>
                                                        {state}
                                                    </option>
                                                ))}
                                            </select>
